$(document).ready(function(){
  ...
  const urlParams = new URLSearchParams(window.location.search); // similar to "?page=Alan+Turing"
  // get the value of the 'page' querystring key
  let page = urlParams.get('page');

  // API URL example from Stack Overflow in the README.md
  $.getJSON("http://en.wikipedia.org/w/api.php?action=parse&format=json&callback=?", {page: "Alan Turing"})
  .done(data => {
    // console.log(data);
  ...
  // We'e going to comment out this timer and move it into our code above.
  // timer = setInterval(putWord, 100);
});
